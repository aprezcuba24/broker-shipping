...
export type CategoriesContextValue = CrudContextValue<Category, CategoryFormValues>
...
export function CategoriesProvider({ children }: { children: ReactNode }) {
  ...
  >({
    ...
    toPatchVariables: (category, values) =>
      category.id ? { categoryId: category.id, data: { name: values.name } } : null,
    toDeleteVariables: (category) => (category.id ? { categoryId: category.id } : null),
  })
