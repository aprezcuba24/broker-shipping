import { brokerFetch, type CustomerSummary } from '@broker/api'
import {
  Button,
  Card,
  CardContent,
...
type ExistingCustomerCheckoutProps = {
...
export function ExistingCustomerCheckout({
  control,
  selectedCustomer,
  onCustomerSelect,
}: ExistingCustomerCheckoutProps) {
  const { watch } = useFormContext<CheckoutOrderFormValues>()
    ...
    <div className="space-y-6">
      ...
      {customerId ? (
        <Card>
          ...
          <CardContent>
            <AddressPicker control={control} customerId={customerId} />
          </CardContent>
        </Card>
      ) : null}
    </div>
