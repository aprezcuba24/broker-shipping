...
async def test_products_require_auth(client: AsyncClient) -> None:
    assert (await client.get("/products/provider/")).status_code == 401
...
async def test_products_with_jwt(
    ...
    r = await client.get("/products/provider/", headers=headers)
...
async def test_products_jwt_without_organization_header_returns_400(
    ...
    u = await user_factory.build()
    r = await client.get("/products/provider/", headers=bearer_headers(user_id=u["id"]))
...
async def test_seller_can_list_products_without_organization_header(
    client: AsyncClient,
    user_factory: UserFactory,
    organization_factory: OrganizationFactory,
) -> None:
